...
either nested dissection nodes, of type \texttt{DORDER\lbt CBLK\lbt NEDI},
leaf nodes, of type \texttt{DORDER\lbt CBLK\lbt LEAF}, or disconnected
components nodes of type \texttt{DORDER\lbt CBLK\lbt DICO}.
In order to facilitate the integration of centralized column block
sub-trees into a global distributed column block tree, the values of
the type flags are the same for the \texttt{Dorder} and \texttt{Order}
data types.
...
\iteme[\texttt{typeval}]
  ...
  \texttt{DORDER\lbt CBLK\lbt NEDI} for a nested dissection node,
  \texttt{DORDER\lbt CBLK\lbt LEAF} for a leaf node, or
  \texttt{DORDER\lbt CBLK\lbt DICO} for disconnected components.
\iteme[\texttt{fathnum}]
...
When resizing takes place within a search loop, it may happen that the
location of the empty slot changes when the mask changes. Hence,
writing data to some empty slot after resizing may result in data
loss. To prevent such case, either resizing must happen before the
hash table index is computed, or the computation must be restarted by
jumping back to the beginning of the loop, as exemplified below.

\begin{lstlisting}[style=language-c]
redo:
  for (hashnum = (vertnum * HASHPRIME) & hashmsk; ; hashnum = (hashnum + 1) & hashmsk) {
    if (hashtab[hashnum].vertnum != vertnum) {    /* If vertex not found           */
      if (hashnbr >= hashmax) {                   /* If hash table full, resize it */
        hashResize (&hashtab, &hashsiz, &hashmax, &hashmsk);
        goto redo;                                /* Recompute hashnum with new mask */
      }

      hashtab[hashnum].vertnum = vertnum;         /* Record new vertex */
      ...
      hashnbr ++;                                 /* One more item added to hash table */
    }
    ...
  }
\end{lstlisting}
