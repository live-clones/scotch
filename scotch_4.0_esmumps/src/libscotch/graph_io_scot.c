...
const char * const          dataptr)              /* No use           */
{
  ...
  if (((intLoad (filegeoptr, &dimnnbr) +          /* Read type and number of geometry items */
        intLoad (filegeoptr, &coornbr)) != 2) ||
       (dimnnbr < 1)                          ||
       (dimnnbr > 3)) {
  ...
  if (coorsortflag != 1)                          /* If geometry data not sorted        */
    qsort ((char *) coorsorttab, coornbr,         /* Sort sort area by ascending labels */
           sizeof (GraphGeomScotSort), (int (*) (const void *, const void *)) graphGeomLoadScot2);
  ...
  if (grafptr->vlbltax != NULL) {                 /* If graph has vertex labels */
    ...
    if (vertsortflag != 1)                        /* If graph data not sorted              */
      qsort ((char *) vertsorttab, grafptr->vertnbr, /* Sort sort area by ascending labels */
             sizeof (GraphGeomScotSort), (int (*) (const void *, const void *)) graphGeomLoadScot2);
  }
...
int
graphGeomLoadScot2 (
const GraphGeomScotSort * const vert0ptr,
const GraphGeomScotSort * const vert1ptr)
{
  return ((vert0ptr->labl > vert1ptr->labl) ?  1 : -1);
}
